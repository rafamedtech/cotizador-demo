...
const dateFormatted = computed(() =>
  new Date(props.invoice.invoiceDate).toLocaleDateString("es-MX", dateOptions),
);
</script>
...
<template>
  ...
  >
    <div
      class="card-body lg:flex-row lg:items-center lg:justify-between lg:gap-0"
    >
      ...
      >
      <h2 class="card-title text-primary lg:w-[25%]">
        {{ invoice.clientCompany }}
      </h2>
      <p class="card-title text-dark-medium dark:text-light-medium lg:w-[25%]">
        <span class="text-primary font-bold lg:hidden">Total:</span>
        {{
